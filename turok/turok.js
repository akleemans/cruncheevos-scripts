...
set.addAchievement({
  ...
  badge: '677412',
  title: 'Nature Lover',
  description: 'Finish the Lost Land without killing any plants or dinosaurs and without dying or leaving the level after collecting the first key - killing Attack Robots or Alien Infantry is allowed',
  points: 5,
  conditions: {
    core: $(
      ...
      // PauseLock: On dying (which will reset enemies) with at least one key collected, lock cheevo
      ['AndNext', 'Mem',   '8bit', nrOfKeysCollected, '>=', 'Value', '',     1],
      ['AddHits', 'Mem',   '8bit', currentLives,      '<',  'Delta', '8bit', currentLives],
      ['PauseIf', 'Value', '',     0,                 '=',  'Value', '',     1,            1],

      // PauseLock 2: On leaving level (which will reset enemies) with at one or two keys collected, lock cheevo
      ['OrNext',  'Mem',   '8bit', nrOfKeysCollected, '=', 'Value', '', 1],
      ['AndNext', 'Mem',   '8bit', nrOfKeysCollected, '=', 'Value', '', 2],
      ['AndNext', 'Delta', '8bit', currentRoomId,     '=', 'Value', '', 0x34],
      ['AddHits', 'Mem',   '8bit', currentRoomId,     '=', 'Value', '', 0x3f],
      ['PauseIf', 'Value', '',     0,                 '=', 'Value', '', 1,    1],

      ...cheatProtection(),
    ),
    alt1: $(
      // Reset if player in Hub with 0 keys
      ['AndNext', 'Mem', '8bit', currentRoomId,     '=', 'Value', '', 0x3f],
      ['ResetIf', 'Mem', '8bit', nrOfKeysCollected, '=', 'Value', '', 0],
    ),
  },
});
